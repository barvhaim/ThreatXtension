...
const SASTSignatureModal = ({
  ...
  useEffect(() => {
    if (
      ...
      generatedSignatures[selectedSignatureIndex]
    ) {
      const sig = generatedSignatures[selectedSignatureIndex];
      setSignatureName(
        sig.rule_id || `custom-${file?.name.replace(/\.[^/.]+$/, "")}`,
      );
      setSignatureDescription(
        sig.message || "Custom SAST rule generated from file analysis",
      );
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        if (signatures.length > 0) {
          setSignatureName(
            signatures[0].rule_id ||
              `custom-${file.name.replace(/\.[^/.]+$/, "")}`,
          );
          setSignatureDescription(
            signatures[0].message ||
              "Custom SAST rule generated from file analysis",
          );
        }
      } else {
      ...
      if (errorMessage.includes("cleaned up") || errorMessage.includes("410")) {
        ...
        errorMessage =
          "Extension files not found. The scan data may have expired. Please re-scan the extension.";
      }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Use AI to automatically generate a custom Semgrep rule for detecting
            similar patterns
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              ...
              <div className="text-center">
                <p className="font-medium">
                  Analyzing file and generating signature...
                </p>
              ...
              <Card className="border-purple-500/50">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-base">
                      Signature {selectedSignatureIndex + 1} of{" "}
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          setSignatureDescription(e.target.value)
                        }
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {generatedSignatures[selectedSignatureIndex]?.metadata
                      ?.category && (
                      <div>
                        <label className="text-sm text-muted-foreground">
                          Category
