...
import { Search, Plus, Edit, Trash2, Copy, Check, FileCode, AlertTriangle } from "lucide-react";
...
const SASTSignaturesPage = () => {
  ...
  const handleEditSignature = (signature) => {
    ...
    setEditForm({
      ...
      languages: Array.isArray(signature.languages) ? signature.languages.join(", ") : signature.languages,
  ...
  const confirmDelete = () => {
    const newSignatures = signatures.filter(s => s.id !== selectedSignature.id);
    ...
    <div className="page-container">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-red-500">
              {signatures.filter(s => s.severity === "ERROR" || s.severity === "CRITICAL").length}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                {searchQuery ? "Try adjusting your search query" : "Create your first SAST signature to get started"}
              </p>
              {!searchQuery && (
                <Button onClick={handleCreateSignature} className="gap-2">
                  <Plus className="h-4 w-4" />
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <CardTitle className="text-lg">{signature.rule_id}</CardTitle>
                    ...
                    <p className="text-sm text-muted-foreground">{signature.message}</p>
                  </div>
                  ...
                  <div className="flex gap-4 text-sm">
                    <div>
                      ...
                      <span className="font-medium">
                        {Array.isArray(signature.languages) ? signature.languages.join(", ") : signature.languages}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Category: </span>
                      <span className="font-medium">{signature.category || "security"}</span>
                    </div>
                    {signature.source_file && (
                      <div>
                        <span className="text-muted-foreground">Source: </span>
                        <span className="font-medium">{signature.source_file}</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <Dialog open={isEditModalOpen || isCreateModalOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {selectedSignature ? "Edit SAST Signature" : "Create New SAST Signature"}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setEditForm({ ...editForm, rule_id: e.target.value })}
      ...
      <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this SAST signature? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedSignature && (
            <div className="p-4 bg-muted rounded-lg">
              ...
              <div className="text-sm text-muted-foreground">{selectedSignature.message}</div>
            </div>
          )}
