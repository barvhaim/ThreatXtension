...
def format_metadata(metadata_data, manifest_data=None):
    ...
    if not metadata_data and manifest_data:
        ...
        formatted += (
            "Chrome Web Store metadata (ratings, user count, etc.) is only available for URLs.\n\n"
        )
    ...
    if not metadata_data and manifest_data:
        ...
        formatted += "\n"
        formatted += (
            "> ⚠️ **Note:** The following information is only available from Chrome Web Store: "
        )
...
def analyze_extension(extension_url: str, uploaded_file, progress=gr.Progress()):
    ...
    try:
        ...
        if uploaded_file is not None:
            ...
            if not (file_path.lower().endswith(".crx") or file_path.lower().endswith(".zip")):
                return ["Please upload a .crx or .zip file", None, None]

            # Use Gradio's temporary file directly
            chrome_extension_path = file_path
            ...
            return [
                "Please provide either a Chrome Web Store URL or upload a .crx/.zip file",
                None,
                None,
            ]
