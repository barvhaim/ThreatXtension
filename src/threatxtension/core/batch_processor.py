...
class BatchProcessor:
    ...
    def process_batch(
        ...
        logger.info("Starting batch %s with %d extensions", batch_id, len(extension_list))
    ...
    def _process_sequential(self, extension_list: List[str], batch_id: str) -> List[Dict]:
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Processing {len(extension_list)} extensions...", total=len(extension_list)
            )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_ext):
                    ...
                    try:
                        ...
                        results.append(result)
                        progress.update(task, description=f"[cyan]Completed: {ext_path[:50]}...")
                        progress.advance(task)
                    except Exception as e:
                        logger.error("Error processing %s: %s", ext_path, e)
                        results.append(
                            {
    ...
    def _analyze_single_extension(self, extension_path: str, batch_id: str, index: int) -> Dict:
        """
        ...
        try:
            ...
            logger.error("Error analyzing %s: %s", extension_path, e, exc_info=True)
            return {
                "extension_path": extension_path,
                "workflow_id": workflow_id,
                "status": "failed",
